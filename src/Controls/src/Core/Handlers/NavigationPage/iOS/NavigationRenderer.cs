...
namespace Microsoft.Maui.Controls.Platform.Compatibility
{
	public class NavigationRenderer : UINavigationController, INativeViewHandler
	{
		...
		public SizeRequest GetDesiredSize(double widthConstraint, double heightConstraint) =>
			NativeView.GetDesiredSize(NavPage, widthConstraint, heightConstraint);
		...
		class ParentingViewController : UIViewController
		{
			...
			{
				UIView nativeView;
				if ((nativeView = Child.GetNative(true)) != null)
			...
			void UpdateToolbarItems()
			{
				...
				foreach (var item in toolbarItems)
				{
					if (item.Order == ToolbarItemOrder.Secondary)
						(secondaries = secondaries ?? new List<UIBarButtonItem>()).Add(item.ToNative(true));
					else
						(primaries = primaries ?? new List<UIBarButtonItem>()).Add(item.ToNative());
				}
		...
		class Container : UIView
		{
			...
			public Container(View view, UINavigationBar bar) : base(bar.Bounds)
			{
				...
				{
					_view = view;
					var platformView = view.ToNative(view.FindMauiContext(), true);
